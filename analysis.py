...
if __name__ == '__main__':
	...
	for key, value in data.items():
		...
		if wage:
			...
			try:
				...
				if wage_2[-3] == '万':
					wage_2 = wage_2[:-3]
					wage = (float(wage_1) * 1e4 + float(wage_2) * 1e4) / 1
				elif wage_2[-3] == '千':
					wage_2 = wage_2[:-3]
					wage = (float(wage_1) * 1e3 + float(wage_2) * 1e3) / 1
				else:
					continue
			except:
				if wage[-3] == '万':
	...
	DrawBar(title='部分城市UI相关岗位平均薪资柱状图', data=wageDict_avg, savepath='./results')
	...
	for key, value in data.items():
		...
		if area not in statistics_area:
			...
			statistics_area[area] += 1
	DrawMap(title='部分地区UI相关岗位招聘需求量', data=statistics_area, savepath='./results')
	...
	DrawPie(title='UI相关岗位有哪些', data=positionDict_new, savepath='./results')